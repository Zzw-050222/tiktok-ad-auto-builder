...
# 发布（含「报错就点修复再发」的重试）2026-08-19 挪到 src/pages/campaign_page.py
# 去共用了：小游戏那边加「多广告组、每组素材不同」时会在一个计划里建好几个广告，
# 撞上同一个平台 bug 的概率更高，两边共用一份免得以后每个 bug 要修两遍。
# 逻辑完全没变，坑的来龙去脉都在那边的注释里。
from src.pages.campaign_page import publish_all  # noqa: E402,F401
