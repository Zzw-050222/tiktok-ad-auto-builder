...
def _reload_ad_page(page):
    ...
    草稿是平台自动保存的（页面底部一直写着「草稿已保存」），所以刷新不丢已填内容。

    原来这里还有一道「刷完如果落到已经填过内容的广告上就中止」的守卫，判据是
    文案/落地页非空。2026-08-20 去掉了，因为那个判据本身是错的：
      * 小游戏的落地页链接【平台会自己带出来】，从一开始就非空
      * 现在的顺序是先写文案和 URL 再复制广告组，所有副本一出生就带着这两项
    也就是说那道守卫会 100% 误报，把能跑的流程全挡下来。真正的防重复现在靠
    step_flow 里的计数（走到第 k 个广告层就挑第 k 个广告的素材）。
    """
