...
def publish_all(page, timeout_seconds=300, max_fix_rounds=5):
    ...
    # 诊断必须写进【异常消息本身】，不能只 print。
    # 使用者是看网页上那块红字的，print 出去的东西只在启动服务的终端里滚过去，
    # 排查时拿不到——2026-08-20 就吃过这个亏：网页上只有「点了 6 轮发布…」一句，
    # 而「修复」这个按钮名到底对不对、弹层里究竟写了什么，全在终端里看不见。
    raise ValueError(
        f"点了 {max_fix_rounds + 1} 轮发布（每次报错都点过「修复」），"
        "页面始终没有跳回计划列表。请手动到后台看这个计划的状态，别重复跑。\n"
        f"当前可见按钮: {_visible_button_names(page)}\n"
        f"弹层文字: {_dialog_texts(page)}\n"
        "（如果上面的按钮里没有「修复」两个字，说明这个平台的报错框按钮不叫这个名，"
        "把这段发我就能改。）"
    )
